...
class DesktopPet(QWidget):
    ...
    def runAction(self):
        ...
        self.timer.start(self.window.GetActivityFrequency())
    ...
    def contextMenuEvent(self, e, ):
        ...
        if(self.pet_status == self.pet_status_list[5]): act5 = cmenu.addAction("Move")
        else: act5 = cmenu.addAction("Rest")
        if self.if_hourlyChime == 1: act6 = cmenu.addAction("Close HourlyChime")
        else: act6 = cmenu.addAction("Open HourlyChime")
        ...
        if action == act1:
            ...
            default = str(local_time.tm_year) + "-" + str(local_time.tm_mon) + "-"+ str(local_time.tm_mday) + " " + str(local_time.tm_hour) + ":" + str(local_time.tm_min) + ":" + str(local_time.tm_sec)

            time_to_add = input_textbox(self.input_talklabel, "Time", "Target time\n(yyyy-mm-dd hh:mm:ss)", self.x()-280, self.y()-350, default)
            ...
            self.ms.add_info('Bwl',
                             [[content, t.toString(Qt.ISODate).replace('T', ' '), str(t.toMSecsSinceEpoch())[0:-3]]])
            textbox(self.talklabel, "Successfully added! you can check it in Console->Schedule", self.x() - 130, self.y() - 55)
